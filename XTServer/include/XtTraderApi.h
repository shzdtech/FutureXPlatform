...
namespace xti
{
    ...
    public:
        ...
        // 查询期权行情
        virtual void reqInstrumentDetail(const char* accountID, int nRequestId) = 0;
        ...
        // @remakr 算法组合单
        virtual void order(const CAlgGroupOrder* orderInfo, int nRequestId) = 0;
        // @remark 算法单
        virtual void order(const CAlgorithmOrder* orderInfo, int nRequestId) = 0;
        // @remark 随机量交易
        virtual void order(const CRandomOrder* orderInfo, int nRequestId) = 0;

        virtual void order(const CIntelligentAlgorithmOrder* orderInfo, int nRequestId) = 0;
